describe('day8', () => {
  ...
  test('part 1 example', () => {
    const findlength = [7, 3, 4, 2];
    const samples = getSamples(exampleInput);
    const seqLengths = samples.flatMap(({ out }) =>
      out.map((seq) => seq.length)
    );
    const found = seqLengths.filter((l) => findlength.includes(l));
  ...
  const numberMap = Object.entries(charMap).reduce((acc, [key, val]) => {
    const nk = [...key]
      .map((ch) => 2 ** (ch.charCodeAt(0) - 97))
      .reduce((acc, v) => acc + v);
    acc[nk] = val;
    return acc;
  }, {});

  const getBits = (sample) => {
    const allIn = String.prototype.concat(...sample.in);

    const oc = {};
    for (let i = 0; i < allIn.length; i++) {
      oc[allIn[i]] = (oc[allIn[i]] || 0) + 1;
    }
    const occurences = Object.entries(oc).reduce(
      (acc, [key, val]) => {
        acc[val].push(key);
        // console.log('adding', key, 'to', val);
        return acc;
      },
      [[], [], [], [], [], [], [], [], [], []]
    );
    ...
    // a = 1, b = 2, c = 4, d = 8, e = 16, f = 32, g = 64
    const bits = {};
    bits[occurences[6][0]] = 2; // b
    bits[occurences[4][0]] = 16; // e
    bits[occurences[9][0]] = 32; // f

    const four = sample.in.find((s) => s.length === 4);
    const seven = sample.in.find((s) => s.length === 3);
    const a = occurences[8].find(
      (v) => four.indexOf(v) === -1 && seven.indexOf(v) !== -1
    );
    bits[a] = 1; // a
    bits[occurences[8].find((v) => v !== a)] = 4; // c
    bits[occurences[7][0]] = 8; // d
    bits[occurences[7][1]] = 64; // g
    return bits;
  };
  ...
  e komt 4 keer voor
  f komt 9 keer voor
  a en c komen 8 keer voor
  a komt niet voor in 4 maar wel in 7
  d en g komen 7 keer voor
  
  */
  test('part 2 example', () => {
    const samples = getSamples(exampleInput);
    const result = samples.map((sample) => {
      const bits = getBits(sample);
      //   console.log({ bits });

      // translate outs
      const translated = sample.out.map((out) => {
        const num = [...out].reduce((acc, ch) => acc + bits[ch], 0);
        return num;
      });
      const numbers = translated.map((str) => numberMap[str]).join('');
      //   console.log({ sample: sample.out, translated, numbers, numberMap });
      return numbers;
    });
...
const input = `caebgd dagc eabgd aebfgc fbdacge edg dg dbgcef eabfd cgeba | gbcdae dagc acgd gd
